...
> **⚠️ 重要（2026-05-31）**  
> 本文档描述的是**后台管理子系统的目标 REST 规范**（`/api/v1/...`），**不等于**当前课设已部署的 `museum` 后端。  
> **联调、测试、前端开发请以《服务器API接口说明（测试用）.md》和 `museum/src/main/java/com/controller/` 源码为准。**  
> 已实现的用户/个人中心接口实际路径为 `/users/*`、`/user_admin/*`，响应字段为 `state` 而非 `code`。
